...
const TESTNET_CONFIG = {
    ...
    proofServer: "http://217.217.253.66:6300",
};
        ...
        wallet.state().pipe(
            Rx.tap((state) => {
                if (state.syncProgress) {
                    console.log(
                        `Sync progress: synced=${state.syncProgress.synced}, percentage=${state.syncProgress.synced}`,
                    );
                }
            }),
            Rx.filter((state) => state.syncProgress?.synced === true),
            Rx.map((s) => s.balances[nativeToken()] ?? BigInt(0)),
            Rx.filter((balance) => balance > BigInt(0)),
...
async function main() {
    ...
    try {
        ...
        let balance = state.balances[nativeToken()] || BigInt(0);
